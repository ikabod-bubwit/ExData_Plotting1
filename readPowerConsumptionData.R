...
readHouseHoldPowerConsumption <- function() {
	...
	# f <- read.csv('testdata.txt',sep=';', colClasses=c("character","character","double","double","double","double","double","double","numeric"), na.string='?')
	...
	fromDate<-strptime('2007-01-02 00:00:00', "%Y-%m-%d %H:%M:%S")
...
# f<-read.csv('testdata.txt',sep=';') %>% select(Date,Time)